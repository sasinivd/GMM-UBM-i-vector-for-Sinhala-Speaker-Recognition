...
if [ $# != 3  ]; then
  echo "Usage: $0 <data-dir> <output-dir> <output2-dir>"
  echo "e.g.: $0 /media/sf_camera_iwatchlife2017/dev_mix dev_data data/train"
  exit 1
fi
...
for reader_dir in $(find -L $src -mindepth 1 -maxdepth 1 -type d | sort); do
  ...
  find -L $reader_dir/ -iname "*.wav" | sort | xargs -I% basename % .wav | \
      awk -v "dir=$reader_dir" '{printf "%s %s/wav/%s.wav\n", $0, dir, $0}' >>$wav_scp|| exit 1
  find -L $reader_dir/ -iname "*.wav" | sort | xargs -I% basename % .wav | \
