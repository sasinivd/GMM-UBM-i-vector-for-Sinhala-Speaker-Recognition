...
if [ $# != 1 ]; then
  ...
  echo " $0 ~/Ucsc/Research/ASR_DATA/mymodel/train"
  exit 1;
fi
...
find $aishell_audio_dir -iname "*.wav" | grep -i "wav" > $train_dir/wav.flist || exit 1;
...
echo "$0: SINHALA data preparation succeeded"
exit 0;
